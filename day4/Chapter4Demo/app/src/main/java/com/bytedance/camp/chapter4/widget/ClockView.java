...
public class ClockView extends View {
        ...
        for (int i = 0; i < unitLinePositions.size(); i++) {
            ...
            if (i % 5 == 0) {
                ...
                canvas.drawPoints(new float[]{
                        linePosition.left, linePosition.top,
                        (linePosition.left + linePosition.right) / 2, (linePosition.top + linePosition.bottom) / 2,
                        linePosition.right, linePosition.bottom,
                        linePosition.left + (linePosition.right - linePosition.left) / 2 * -2, linePosition.top + (linePosition.bottom - linePosition.top) / 2 * -2,
                        linePosition.left + (linePosition.right - linePosition.left) / 2 * -1, linePosition.top + (linePosition.bottom - linePosition.top) / 2 * -1
    ...
    private void drawTimeNeedles(Canvas canvas) {
        ...
        Log.i("TAG","time:" + hour + ":" + minute + ":" + second);
        float secondDegree = second * 6;
        float endX = (float) (centerX + radius * SECOND_NEEDLE_LENGTH_RATIO * Math.cos(Math.toRadians(secondDegree - 90)));
        float endY = (float) (centerY + radius * SECOND_NEEDLE_LENGTH_RATIO * Math.sin(Math.toRadians(secondDegree - 90)));
        ...
        float minuteDegree = minute * 6 + secondDegree / 360;
        endX = (float) (centerX + radius * MINUTE_NEEDLE_LENGTH_RATIO * Math.cos(Math.toRadians(minuteDegree - 90)));
        endY = (float) (centerY + radius * MINUTE_NEEDLE_LENGTH_RATIO * Math.sin(Math.toRadians(minuteDegree - 90)));
        needlePaint.setColor(Color.WHITE);
        needlePaint.setStrokeWidth(MINUTE_NEEDLE_WIDTH);
        canvas.drawLine(centerX,centerY,endX,endY,needlePaint);

        float hourDegree = hour * 6 + minuteDegree / 360;
        endX = (float) (centerX + radius * HOUR_NEEDLE_LENGTH_RATIO * Math.cos(Math.toRadians(hourDegree - 90)));
        endY = (float) (centerY + radius * HOUR_NEEDLE_LENGTH_RATIO * Math.sin(Math.toRadians(hourDegree - 90)));
        needlePaint.setColor(Color.WHITE);
        needlePaint.setStrokeWidth(HOUR_NEEDLE_WIDTH);
        canvas.drawLine(centerX,centerY,endX,endY,needlePaint);
        Log.i("TAG","timeDegree:" + hourDegree + ":" + minuteDegree + ":" + secondDegree);
    }
    ...
    private void drawTimeNumbers(Canvas canvas) {
        ...
        for (int i = 2; i < 14; i++){
            ...
            numberPaint.getTextBounds(str,0,str.length(),rect);
            startX = (float) (centerX + (radius * (1 - 0.2f)) * Math.cos(Math.toRadians((i-2)*30)));
            startY = (float) (centerY + rect.height() / 2 + (radius * (1 - 0.2f)) * Math.sin(Math.toRadians((i-2)*30)));
            canvas.drawText(str,startX,startY,numberPaint);
        }
    }
